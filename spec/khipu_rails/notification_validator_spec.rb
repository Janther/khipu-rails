# encoding: utf-8
require 'spec_helper'

describe KhipuRails::NotificationValidator do
  before :all do
    @notification ||= {
      api_version: "1.2",
      receiver_id: "1392",
      notification_id: "dfs40ivmw7fz",
      subject: "Compra de Puntos Cumplo",
      amount: "2182",
      currency: "CLP",
      transaction_id: "",
      payer_email: "klahott@gmail.com",
      custom: "",
      notification_signature: ""
    }

    KhipuRails.config = nil
    KhipuRails.configure do |config|
      config.receivers.merge! "1392" => 'b174c94de0ec3ce4f1c3156e309de45e8ce0f9ef'
    end
  end

  it "validates locally a valid transaction" do
    KhipuRails::NotificationValidator.validate(@notification, :local).should == true
  end

  it "validates a valid transaction via webserver" do
    KhipuRails::NotificationValidator.validate(@notification, :webservice).should == true
  end
end